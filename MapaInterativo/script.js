...
const currencyOutput = document.querySelector(".moeda");
...
countries.forEach(country => {
    ...
    country.addEventListener("click", function(e){
        ...
        const API_KEY = "rc_live_33c92e0602b14494afe239e711a62b86";

fetch(
    `https://restcountries.com/v3.1/name/${encodeURIComponent(clickedCountryName)}?fullText=true&apikey=${API_KEY}`
).then(response => {
            if(!response.ok){
                throw new Error('Sinal não correspondido');
            }
        ...
        .catch(error => {
            loading.innerText = "Sem dados para este país";
            console.log("Há um problema com a operação fetch");
        });
    });
});
